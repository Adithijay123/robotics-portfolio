"""
PPO agent navigating UR5e through Z-maze in MuJoCo.
Same maze, arm, start/goal, reward logic, and output format as ql_zmaze.py.
IDENTICAL to ppo_umaze.py — only maze geometry, wall names, and file
labels changed to Z-maze. Rendering ENABLED.
References:
...
#   OUTPUT
existing       = [f for f in os.listdir(OUTPUT_DIR) if f.startswith("zsimppo_trial")]
run_num        = len(existing) // 3 + 1
TRIAL_CSV      = os.path.join(OUTPUT_DIR, f"zsimppo_trial{run_num}.csv")
SUMMARY_PATH   = os.path.join(OUTPUT_DIR, f"zsimppo_trial{run_num}_summary.csv")
MODEL_PATH_OUT = os.path.join(OUTPUT_DIR, f"zsimppo_trial{run_num}_model.pt")
summaries      = []
_trial_hdr     = False
...
#   TRAINING LOOP
print(f"PPO | Z-maze | {EPISODES} eps | step={STEP_SIZE*1000:.0f}mm | obs_dim={OBS_DIM}")
