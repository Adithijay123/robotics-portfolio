"""
ql_umaze.py
Q-Learning agent navigating UR5e through U-maze in MuJoCo with a probe to subsitube for the pizoelectric sensor.
Real Lego dimensions: 16mm corridor, 4mm probe (2mm radius), 6mm clearance.
"""
...
EPISODES           = 500
MAX_STEPS          = 500
STEP_SIZE          = 0.005     # 5mm per step — fits in 16mm corridor
GOAL_THRESH        = 0.012     # 12mm radius
ALPHA              = 0.20
GAMMA              = 0.95
EPSILON_START      = 1.0
EPSILON_END        = 0.05
...
HOME_TARGET = np.array([-0.154, -0.325, 0.16642])  # green site — left arm
GOAL_TARGET = np.array([-0.026, -0.325, 0.16635])  # red site  — right arm same Y as start
...
# Discretisation: fixed bounding box covering entire maze
X_MIN, X_MAX = -0.175, -0.005   # outer wall to outer wall in X
Y_MIN, Y_MAX = -0.400, -0.308   # base outer to back caps in Y
X_SPAN = X_MAX - X_MIN
Y_SPAN = Y_MAX - Y_MIN
...
#   OUTPUT                                   
# Auto-increment run number based on existing files
existing = [f for f in os.listdir(OUTPUT_DIR) if f.startswith("usimql_trial")]
run_num  = len(existing) // 3 + 1   # 3 files per run (trials, summary, qtable)
TRIAL_CSV    = os.path.join(OUTPUT_DIR, f"usimql_trial{run_num}.csv")
SUMMARY_PATH = os.path.join(OUTPUT_DIR, f"usimql_trial{run_num}_summary.csv")
QTABLE_PATH  = os.path.join(OUTPUT_DIR, f"usimql_trial{run_num}_qtable.pkl")
summaries    = []
_trial_hdr   = False
...
print(f"Q-Learning | U-maze | {EPISODES} eps | step={STEP_SIZE*1000:.0f}mm | bins={N_BINS}x{N_BINS}")
...
print(f"  Q-Learning U-maze — Training Complete")
